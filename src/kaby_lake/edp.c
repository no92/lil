...
bool kbl_edp_aux_readable(LilGpu *gpu, LilConnector *con) {
	...
	if(REG(PP_CONTROL) & 8) {
		...
		if(kbl_ddi_hotplug_detected(gpu, con->ddi_id)) {
			return dp_aux_native_read(gpu, con, DPCD_REV) != 0;
		}
	} else {
		REG(PP_CONTROL) |= 8;
		lil_sleep(10);
		...
		if(enc->edp.t3_optimization) {
			...
			while(tries --> 0) {
				if(kbl_ddi_hotplug_detected(gpu, con->ddi_id)) {
					...
					if(dp_aux_native_read(gpu, con, DPCD_REV) != 0) {
						return true;
					}
				}
...
bool kbl_edp_pre_enable(LilGpu *gpu, LilConnector *con) {
	...
	if(enc->edp.edp_fast_link_training_supported) {
		...
		for(size_t i = 0; i < 8; ++i) {
			uint16_t val = 0;
			dp_aux_native_readn(gpu, con, 2 * (i + 8), 2, &val);
		...
		enc->edp.edp_max_link_rate = dp_aux_native_read(gpu, con, MAX_LINK_RATE);
	}

	enc->edp.edp_lane_count = dp_aux_native_read(gpu, con, MAX_LANE_COUNT);
