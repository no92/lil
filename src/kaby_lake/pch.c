...
void kbl_pch_get_gen(LilGpu *gpu) {
	uint16_t devid = lil_pci_readw(gpu->pch_dev, 2);

	if(in_pciid_list(devid, LPT_PCIIDS, sizeof(LPT_PCIIDS)))
		gpu->pch_gen = LPT;
	else if(in_pciid_list(devid, LPT_LP_PCIIDS, sizeof(LPT_LP_PCIIDS)))
		gpu->pch_gen = LPT_LP;
	else if(in_pciid_list(devid, SPT_LP_PCIIDS, sizeof(SPT_LP_PCIIDS)))
		gpu->pch_gen = SPT_LP;
	else if(in_pciid_list(devid, SPT_PCIIDS, sizeof(SPT_PCIIDS)))
		gpu->pch_gen = SPT;
	else if(in_pciid_list(devid, KBP_PCIIDS, sizeof(KBP_PCIIDS)))
		gpu->pch_gen = KBP;
