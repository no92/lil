...
int main(int argc, char *argv[]) {
	...
	if(statbuf.st_size) {
		...
		while((ret = read(fd, vbt + len, size - len))) {
			if(ret < 0) {
...
static const char *child_device_type_bits[16] = {
...
static const char *device_handles[] = {
...
static const char *device_handle(uint8_t handle) {
	for(size_t i = 0; i < ARRAY_LEN(device_handles); i++) {
		if(device_handles[i] && handle == i) {
			return device_handles[i];
		}
	}
...
static void dump_block(const struct vbt_header *hdr, int block) {
	const struct bdb_block_header *b = vbt_get_bdb_block(hdr, block);
	...
	switch(block) {
		case BDB_GENERAL_DEFINITIONS: {
			const struct bdb_general_definitions *d = (void *) b;
			...
			for(size_t child = 0; child < children; child++) {
				struct child_device *dev = (void *) ((uintptr_t) &d->child_dev + (child * d->child_dev_size));
		...
		case BDB_OEM_MODE: {
			const struct bdb_oem_mode *oem = (void *) b;
		...
		case BDB_FIXED_MODE_SET: {
			const struct bdb_fixed_mode_set *f = (void *) b;
			if(f->feature_enable) {
		...
		case BDB_PRD_BOOT_TABLE: {
			const struct bdb_prd_boot_table *prd = (void *) b;
